...
const initiallyDisplayedCurrencies = ["USD", "EUR", "GBP", "NGN"];
let baseCurrency;
...
let currencies = [
  ...
  },
  {
    ...
    abbreviation: "NGN",
    symbol: "\u20A6",
    flagURL: "http://www.geonames.org/flags/l/ng.gif"
  },
  {
    name: "Japanese Yen",
    abbreviation: "JPY",
  ...
  for (let i = 0; i < currencies.length; i++) {
    addCurrencyList.insertAdjacentHTML(
      ...
      `<li data-currency=${currencies[i].abbreviation}>
            <img src=${currencies[i].flagURL} class="flag" alt=""><span>${currencies[i].abbreviation} - ${currencies[i].name}</span>
        </li>`
    );
  }
}
...
function newCurrenciesListItem(currency) {
  ...
  const baseCurrencyRate = currencies.find(c => c.abbreviation === baseCurrency)
    .rate;
  const exchangeRate =
